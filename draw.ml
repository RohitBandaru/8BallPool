...
let draw_state canvas =
  List.iter (fun b -> (draw_ball canvas b)) (ball_locations !cur_state)

let draw_stick canvas =
  let cue_pos = get_position (List.find (fun b -> get_id b = 0 ) (ball_locations !cur_state)) in
...
let update_cue_pos position =
  (* TODO update the state with new ball position *)
  ()
...
let rec loop canvas =
  ...
  let balls = ball_locations !cur_state in
  let tdelta = 0.001 in
